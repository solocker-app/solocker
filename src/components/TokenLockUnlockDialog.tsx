...
export default function TokenUnlockDialog({
  ...
  const onConfirm = function () {
    ...
    return toast.promise(
      repository.streamflow.withdraw(
        id,
        stream.depositedAmount - stream.withdrawnAmount,
        info.lpTokenDecimal,
      ),
      {
        pending: "Unlocking liquidity token",
        success: "Liquidity token unlocked successfully",
      ...
      <div className="w-xs bg-container rounded-md md:w-sm">
        <header className="flex p-4">
          <div className="flex-1 flex flex-col">
            <h1 className="text-xl font-bold">Unlock Liquidity</h1>
            <p className="text-highlight">
              Do you want to unlock your liquidity tokens?
            </p>
          </div>
          <div>
